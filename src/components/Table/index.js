...
export default {
  ...
  render() {
    ...
    // columns如果配置hidden，则过滤到该列不显示
    props.columns = props.columns.filter((item) => !item.hidden)
    const table = (
      <a-table
      ...
      >
        {props.columns.map((item) => {
          if (item.slots) {
            return (
              <template slot={item.slots.title}>
                {item.slots.name}
                <a-tooltip placement="top">
                  <template slot="title">{item.slots.desc}</template>
