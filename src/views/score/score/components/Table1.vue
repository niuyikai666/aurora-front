<template>
  <div>
    <div>
      <total :api="actorScoreBaseStatistic" :params="params" />
    </div>
...
import { getScoreAdminList, actorScoreBaseStatistic, getScoreAdminDelete } from '@/api/score'
...
export default {
  ...
  methods: {
    getData (parameter) {
      ...
      return getScoreAdminList(params).then(res => {
        return res
      }).catch(code => {
        if (code === 405) {
          this.$emit('unauthority')
        }
      })
    },
    detail (data) {
    ...
    downloadHandle () {
      ...
      window.location.href = `${process.env.VUE_APP_API_BASE_URL}/actor/score/actorScore/admin/download${url}`
    }
  },
  filters: {
    changeCategory (val) {
