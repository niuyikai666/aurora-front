...
import { columnsSquared } from '../tableColumns'
import { getSquaredList } from '@/api/commission'
...
export default {
  ...
  data () {
    return {
      ...
      amountFormat,
      columns: columnsSquared(this.type),
      queryParams: {}
    }
  },
  mounted () {
  ...
  methods: {
    ...
    download () {
      const path = `${process.env.VUE_APP_API_BASE_URL}/actor/task/commission/exportCommissionDetailVideoPalaceList?type=${this.type}&monthCycle=${this.params ? this.params.monthCycle : null}${this.queryParams.search ? ('&search=' + this.queryParams.search) : ''}`
