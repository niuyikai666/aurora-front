<template>
  ...
  >
    ...
    <template slot="footer">
      <a-button class="footer-btn" @click="cancel">取消</a-button>
      <a-button class="footer-btn" :loading="loading" type="primary" @click="submit('loading')">提交申请仲裁</a-button>
    </template>
  </a-modal>
</template>
...
import { applyDetail, applicationHandle } from '@/api/actived'
...
export default {
  ...
  methods: {
    ...
    submit (text) {
      this[text] = true
      applicationHandle(this.id).then(res => {
        this[text] = false
