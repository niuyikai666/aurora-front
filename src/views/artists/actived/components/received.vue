  ...
  <div class="pd24">
    ...
    >
      ...
      <template slot="action" slot-scope="text, record">
        ...
        <a-button class="mr10" type="link" @click="showDealModal(record.id)" v-if="record.state.code === 2 && permission.includes('handle_examine')">审核</a-button>
      </template>
    </s-table>
    <detail-modal :id="id" :visible="visible" @cancel="visible = false"/>
    <receive-modal :id="id" :visible="dealVisible" @cancel="dealVisible = false" />
  </div>
</template>
...
import { getReceiveList } from '@/api/actived'
import { mapGetters } from 'vuex'
import { STable } from '@/components'
import { receiveColumns } from '../tableColumns'
export default {
  components: {
    STable,
    detailModal,
  ...
  methods: {
    loadData (params) {
      ...
      return getReceiveList(params).then(res => {
        return res
      })
    },
    resetFormFileds () {
    ...
    download () {
      this.getParams().then(res => {
        ...
        path += `/activation/receivedActivation/export${url}`
