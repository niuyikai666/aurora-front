...
import { interruptColumns } from '../tableColumns'
export default {
  props: {
    fn: {
      type: Function,
  ...
  data () {
    return {
      ...
      colunmsChecked: [],
      checkArr: [],
      defaultColunm: ['nickName', 'currentMonthTotalReward', 'lastLiveTime', 'signMethod', 'rewardLastMonth', 'breakLiveDays', 'action']
    }
  },
  created () {
  ...
  methods: {
    ...
    getTableColumns () {
      ...
      this.tableColums = interruptColumns.filter(item => storageColumns.includes(item.dataIndex) || this.defaultColunm.includes(item.dataIndex))
      this.colunmsChecked = storageColumns
      this.checkArr = interruptColumns.filter(item => !this.defaultColunm.includes(item.dataIndex) && item.dataIndex !== 'action')
    },
    checkedChange () {
      this.tableColums = interruptColumns.filter(item => this.colunmsChecked.includes(item.dataIndex) || this.defaultColunm.includes(item.dataIndex))
