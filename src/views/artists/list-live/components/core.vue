...
import { coreColumns } from '../tableColumns'
export default {
  props: {
    fn: {
      type: Function,
  ...
  methods: {
    ...
    getTableColumns () {
      ...
      this.tableColums = coreColumns.filter(item => storageColumns.includes(item.dataIndex) || this.defaultColunm.includes(item.dataIndex))
      this.colunmsChecked = storageColumns
      this.checkArr = coreColumns.filter(item => !this.defaultColunm.includes(item.dataIndex) && item.dataIndex !== 'action')
    },
    checkedChange () {
      this.tableColums = coreColumns.filter(item => this.colunmsChecked.includes(item.dataIndex) || this.defaultColunm.includes(item.dataIndex))
