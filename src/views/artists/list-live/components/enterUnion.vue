...
import { EnterUnionColumns } from '../tableColumns'
export default {
  props: {
    fn: {
      type: Function,
  ...
  methods: {
    ...
    getTableColumns () {
      ...
      this.tableColums = EnterUnionColumns.filter(item => storageColumns.includes(item.dataIndex) || this.defaultColunm.includes(item.dataIndex))
      this.colunmsChecked = storageColumns
      this.checkArr = EnterUnionColumns.filter(item => !this.defaultColunm.includes(item.dataIndex) && item.dataIndex !== 'action')
    },
    checkedChange () {
      this.tableColums = EnterUnionColumns.filter(item => this.colunmsChecked.includes(item.dataIndex) || this.defaultColunm.includes(item.dataIndex))
