        ...
        <a-row class="sign-content">
          ...
          <a-col :xs="24" :md="8">
            <a-form-item
              ...
              <a-input-number placeholder="请输入短视频介入孵化时粉丝量" v-decorator="['operationInitFans']" />
            </a-form-item>
            <a-form-item
              label="短视频介入孵化时粉丝量"
              v-if="signModel.operationInitFans !=null">
      ...
      <tab-content title="策划信息" :titleLine="true">
        <a-row class="sign-content">
          ...
          <a-col :xs="24" :md="8">
            <a-form-item
              label="归属团队">
              <p class="mb0">{{ signModel.departmentName ? signModel.departmentName : '--' }}</p>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="8">
            <a-form-item
        ...
        <a-row class="sign-content">
          ...
          <a-col :xs="24" :md="8">
            ...
            <a-form-item
              v-if="signModel.operationStartDate"
              label="短视频接手时间">
              <p class="crip" style="margin-bottom:0">{{ signModel.operationStartDate }}</p>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="8">
            <a-form-item
...
<script>
import { editMeDiatMessage, artistSearch } from '@/api/goldData'
...
export default {
  ...
  methods: {
    getSignInfoHandle () {
      this.res.operationType = this.res.operationType ? this.res.operationType.code : undefined
      this.res.operationMethod = this.res.operationMethod ? this.res.operationMethod.code : undefined
      this.res.departmentName = this.res.engineerEmployeeDepartment ? this.res.engineerEmployeeDepartment : undefined
    ...
    dataSearchHandle (query, s) {
      this.artistSearchHandle(query).then(res => {
        this.dataSource = res
      })
    },
    async dataSearchHandle1 (query, s) {
      const data = await this.artistSearchHandle(query)
      this.dataSource1 = data
    },
    async dataSearchHandle2 (query, s) {
      const data = await this.artistSearchHandle(query)
      this.dataSource2 = data
    },
    async dataSearchHandle3 (query, s) {
      const data = await this.artistSearchHandle(query)
      this.dataSource3 = data
    },
    async dataSearchHandle4 (query, s) {
      const data = await this.artistSearchHandle(query)
      this.dataSource4 = data
    },

    // **优化 提取搜索接口
    async artistSearchHandle (query) {
      let data = []
      data = await artistSearch({ name: query, dutyType: 2 }).then(res => {
        if (res.length > 0) {
          res = res.map(item => {
            item.id = item.id + ''
            return item
    ...
    setCeHuaData (data) {
      if (!data && !this.signModel.engineerEmployeeId) {
        this.$message.error('短视频策划不存在')
        this.engineerEmployeeId = ''
        return
      }
      this.signModel.engineerEmployeeId = data.id
      this.signModel.departmentName = data.fullName
    },
    onCeHuaBlur (value) {
      if (!value) {
        return false
