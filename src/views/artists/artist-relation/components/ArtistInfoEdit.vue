        ...
        <a-row class="sign-content">
          <!-- <a-col :xs="24" :md="8">
            <description title="直播平台账号昵称">
              <p class="crip">{{ signModel.nickname ? signModel.nickname : '--' }}</p>
            </description>
          </a-col>
          <a-col :xs="24" :md="8">
            <description title="抖音号">
              <p class="crip">{{ signModel.newTrillNumber ? signModel.newTrillNumber : '--' }}</p>
            </description>
          </a-col>
          <a-col :xs="24" :md="8">
            <description title="抖音号（原）">
              <p class="crip">{{ signModel.oldTrillNumber ? signModel.oldTrillNumber : '--' }}</p>
            </description>
          </a-col> -->
          <a-col :xs="24" :md="8">
            <a-form-item
        ...
        <a-row class="sign-content">
          ...
          <!-- <a-col :xs="24" :md="8">
            <a-form-item
              label="主播类型">
              <a-select
                v-decorator="['influencerType',{rules: [{ required: true, message: '请选择主播类型'}]}]"
                mode="multiple"
                placeholder="请选择平台"
              >
                <a-select-option :value="item" v-for="(item, index) in influencerType" :key="index">
                  {{ item }}
                </a-select-option>
              </a-select>
            </a-form-item>
        ...
        <a-row class="sign-content">
          <!-- <a-col :xs="24" :md="8">
            <a-form-item
              ...
              <a-select
                ...
                placeholder="请选择"
              >
                <a-select-option v-for="(item, index) in signMethod" :key="index" :value="item.value">
                  {{ item.name }}
                </a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col :xs="24" :md="8">
            <a-form-item
              label="招募人为无忧员工">
              <a-select
                :disabled="signInfoDisabled || !isDays"
                v-decorator="['isInRecruit',{rules: [{ required: !(signInfoDisabled || !isDays), message: '请选择是否为无忧员工'}]}]"
                @change="change"
                placeholder="请选择"
              >
          ...
          <a-col :xs="24" :md="8">
            <a-form-item
              label="招募姓名">
              <a-input :disabled="signInfoDisabled || !isDays" placeholder="请输入招募姓名" v-decorator="['recruitName',{rules: [{ required: !(signInfoDisabled || !isDays), message: '请输入招募姓名',pattern: /^(?=.*\S).+$/}]}]" v-if="showRecruitType!=1"/>
              <a-auto-complete
                :disabled="signInfoDisabled || !isDays"
                style="width: 100%"
                placeholder="请选择招募人"
                option-label-prop="title"
                @search="allSearchHandle"
                @select="onSelect1"
                @blur="onBlur1"
                v-if="showRecruitType==1"
                v-decorator="['recruitName',{rules: [{ required: !(signInfoDisabled || !isDays), message: '请选择招募人'}]}]"
              >
                <template slot="dataSource">
                  <a-select-option v-for="item in allSource" :key="item.id" :title="item.name">
                    <span>{{ item.name }}({{ item.employeeNo }}){{ item.fullName }}</span>
                  </a-select-option>
                </template>
          ...
          <a-col :xs="24" :md="8" v-if="!showRecruitType">
            <a-form-item
              label="招募手机号">
              <a-input :disabled="signInfoDisabled || !isDays" placeholder="请输入招募手机号" v-decorator="['recruitMobile',{rules: [{ required: false, message: '请输入正确招募手机号', pattern: /^1[123456789]\d{9}$/}]}]" />
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="8">
            <a-form-item
          ...
          <!-- <a-col :xs="24" :md="8">
            <a-form-item
              ...
              <a-auto-complete
                ...
                @select="onSelect2"
                @blur="onBlur2"
                v-decorator="['signedEmpName',{rules: [{ required: true, message: '请选择签约人'}]}]"
              >
                <template slot="dataSource">
                  <a-select-option v-for="item in signSource" :key="item.id" :title="item.name">
          ...
          <a-col :xs="24" :md="8">
            <a-form-item
              label="运营人分公司">
              <span>{{ signModel.fullName ? signModel.fullName : '--' }}</span>
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="8">
            <a-form-item
              ...
              <a-select
                ...
                :disabled="lecturerRecruitNameDisabled"
                placeholder="请选择"
              >
          ...
          <a-col :xs="24" :md="8" v-show="form.getFieldValue('isLecturerRecruit')">
            <a-form-item
              v-if="form.getFieldValue('isLecturerRecruit')"
              label="讲师姓名">
              <a-auto-complete
                style="width: 100%"
                placeholder="请选择讲师姓名"
          ...
          <a-col :xs="24" :md="8">
            <description title="入驻时间">
              <p class="crip">{{ signModel.signingTime ? signModel.signingTime : '--' }}</p>
            </description>
          </a-col>
        </a-row>
      </tab-content>
...
import { editArtistMessage, artistSearch, getStructure } from '@/api/goldData'
import { artistSearch2, artistSignSearch, checkInfluncerDays } from '@/api/gold'
...
export default {
  ...
  data () {
    return {
      ...
      signInfoDisabled: false,
      lecturerRecruitNameDisabled: false,
      isDays: true
    }
  },
  created () {
  ...
  mounted () {
    ...
    this.getInfoHandle()
    // this.getStructureHandle()
  },
  methods: {
    getInfoHandle () {
        this.res.influencerType = this.res.influencerType ? this.res.influencerType.split(',') : []
        this.signModel = this.res
        this.indexPicUrl = this.res.indexPicUrl
        this.actorPic = this.res.actorPic || '/files/aurora/da6ba4a606e54141ad08e717d0149269.png'
        this.signModel.actorPic = this.actorPic
        this.signModel.isInRecruit = this.signModel.isInRecruit !== null ? this.signModel.isInRecruit : null
        this.showRecruitType = this.signModel.isInRecruit === 1
        this.signModel.addressCode = this.signModel.addressCode ? this.signModel.addressCode.split(',') : []
        this.address = this.signModel.address
        this.signModel.liveType = this.signModel.liveType ? this.signModel.liveType.code : undefined
        this.signModel.acquirement = this.signModel.acquirement ? this.signModel.acquirement.code : undefined
        this.signModel.appearance = this.signModel.appearance ? this.signModel.appearance.code : undefined
        this.signModel.interaction = this.signModel.interaction ? this.signModel.interaction.code : undefined
        this.signModel.monthAmountType = this.signModel.monthAmountType ? this.signModel.monthAmountType.code : undefined

        this.signModel.talentCategory = this.signModel.talentCategory ? this.signModel.talentCategory.code : undefined
        this.signModel.actorCategory = this.signModel.actorCategory ? this.signModel.actorCategory.code : undefined
        this.signModel.qualification = this.signModel.qualification ? this.signModel.qualification.code : undefined
        this.signModel.bonusSkill = this.signModel.bonusSkill ? this.signModel.bonusSkill.code : undefined
        this.signModel.isLecturerRecruit = this.signModel.isLecturerRecruit !== null ? this.signModel.isLecturerRecruit : undefined

        this.signModel.signMethod = this.signModel.signMethod ? this.signModel.signMethod.code : undefined
        this.signModel && this.form.setFieldsValue(pick(this.signModel, this.fileds))
        this.contractAttachUrl = this.signModel.contractAttach

        this.signInfoDisabled = !!this.res.recruitName
        this.lecturerRecruitNameDisabled = !!this.res.lecturerRecruitEmployeeId
        const data = this.res.newTrillNumber || this.res.volcanoCode
        this.checkDays(data)
    },
    handleSubmit () {
      setTimeout(() => {
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            if (this.showRecruitType) {
              ...
              if (!this.signModel.recruitEmployeeId && this.isDays) {
            ...
            // if (!this.signModel.signedEmployeeId) {
            ...
            editArtistMessage(params).then(res => {
    ...
    operateSearchHandle (query) {
      artistSearch2({ name: query }).then(res => {
        this.operateSource = []
        if (res.length > 0) {
    ...
    setData (data) {
      if (!data) {
        this.$message.error('该运营人不存在')
        this.signModel.operatorEmployeeId = ''
        return
      }
