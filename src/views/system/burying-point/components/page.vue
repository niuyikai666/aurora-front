<template>
  ...
  >
    <template slot="extra">
      <a-range-picker v-model="searchDate" :allowClear="false" @change="searchHandle" value-format="YYYY-MM-DD"/>
    </template>
    <s-table
      :columns="colunms"
    ...
    <record-modal :type="2" :data="data" :visible="visible" @cancel="visible = false" v-if="visible"/>
  </a-card>
</template>

<script>
import moment from 'moment'
import { getPageList } from '@/api/burying-point'
import recordModal from './recordModal'
import { STable } from '@/components'
import { pageColumns } from '../tableColumns'
export default {
  components: {
    STable,
    recordModal
