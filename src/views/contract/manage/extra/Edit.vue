  ...
  <div class="container">
    ...
    >
      <a-form class="form" :form="form">
        ...
        <a-row>
          <a-col>
            <a-button type="primary" @click="submit()">确认编辑</a-button>
          </a-col>
        </a-row>
        </a-row>
      </a-form>
...
<script>
import { extraContractEdit, extraContractDetail } from '@/api/contract'
...
const fileds = [
	'type',
	'liveDayMonthLimit',
	'liveDurationMonthLimit',
	'videoCountMonthLimit',
	'validityDate',
	'digest',
	'remark',
	'isMinReward'
  ...
  methods: {
    ...
    generateGuarant (data) {
      if (!data || data.length <= 0) {
        const blankData = [JSON.parse(JSON.stringify(this.blankData))]
        return blankData
      }
      data.forEach(item => {
    ...
    signTypeChange (val) {
      ...
      if (val !== 1) {
        ...
        this.rbookletPbProp = ''
        this.rbookletPaProp = ''
        this.guaranteedData = [JSON.parse(JSON.stringify(this.blankData))]
      }
    },
