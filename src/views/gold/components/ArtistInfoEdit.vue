        ...
        <tab-content title="身份信息" :titleLine="true">
          <a-row class="sign-content">
            ...
            <a-col :xs="24" :md="8">
              <a-form-item
                label="联系电话">
                <a-input placeholder="请输入主播的手机号" v-decorator="['tel',{rules: [{ required: true, message: '请输入正确手机号', pattern: /^1[123456789]\d{9}$/}]}]" />
              </a-form-item>
            </a-col>
          </a-row>
        </tab-content>
          ...
          <a-row class="sign-content">
            ...
            <a-col :xs="24" :md="8">
              <a-form-item
                ...
                <a-select
                  :disabled="!isDays"
                  v-decorator="['isInRecruit',{rules: [{ required: isDays, message: '请选择是否为无忧员工'}]}]"
                  @change="change"
                  placeholder="请选择"
                >
            ...
            <a-col :xs="24" :md="8">
              <a-form-item
                label="招募姓名">
                <a-input :disabled="!isDays" placeholder="请输入招募姓名" v-decorator="['recruitName',{rules: [{ required: isDays, message: '请输入招募姓名',pattern: /^(?=.*\S).+$/}]}]" v-if="showRecruitType!=1"/>
                <a-auto-complete
                  style="width: 100%"
                  placeholder="请选择招募人"
                  ...
                  :disabled="!isDays"
                  v-decorator="['recruitName',{rules: [{ required: isDays, message: '请选择招募人'}]}]"
                >
                  <template slot="dataSource">
                    <a-select-option v-for="item in allSource" :key="item.id" :title="item.name">
                      <p style="margin:0">{{ item.name }}({{ item.employeeNo }})<span v-if="item.fullName">{{ item.fullName }}</span></p>
                    </a-select-option>
                  </template>
            ...
            <a-col :xs="24" :md="8" v-if="!showRecruitType">
              <a-form-item
                label="招募手机号">
                <a-input :disabled="!isDays" placeholder="请输入招募手机号" v-decorator="['recruitMobile',{rules: [{ required: false, message: '请输入正确招募手机号', pattern: /^1[12345789]\d{9}$/}]}]" />
              </a-form-item>
            </a-col>
            <a-col :xs="24" :md="8">
              <a-form-item
            ...
            <a-col :xs="24" :md="8">
              <a-form-item
                ...
                <a-select
                  ...
                  placeholder="请选择"
                >
                  <a-select-option v-for="(item, index) in isInTeacher" :key="index" :value="Number(item.value)">
                    {{ item.name }}
                  </a-select-option>
                </a-select>
              </a-form-item>
            ...
            <a-col :xs="24" :md="8">
              <a-form-item
                label="签约人">
                <span>{{ signModel.signEmployeeName ? signModel.signEmployeeName : '--' }}</span>
              </a-form-item>
            </a-col>
            <a-col :xs="24" :md="8">
...
import { getArtisInfo, editArtistMessage, artistSearch, artistSignSearch, checkInfluncerDays } from '@/api/gold'
// import { getStructure } from '@/api/report'
  ...
  methods: {
    getInfoHandle () {
      getArtisInfo(this.id).then(res => {
        // videoCountOfMonth
        res.influencerType = res.influencerType ? res.influencerType.split(',') : []
        this.signModel = res
        ...
        console.log(this.signModel.isLecturerRecruit)
        this.operatorDepartmentId = this.signModel.departmentTreeIds
        ...
        this.loading = false
        const data = this.signModel.newTrillNumber || this.signModel.volcanoCode
        this.checkDays(data)
      })
    },
    handleSubmit () {
