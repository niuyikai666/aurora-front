      ...
      <a-skeleton :loading="loading1">
        <a-row class="static-content" style="display:block">
          ...
          <a-col :sm="24" :md="12" :xl="6">
            <target-data
              title="核心主播流水（元）"
              :newData="indexData.current.coreActorRewardMoney"
              :oldData="indexData.last ? indexData.last.coreActorRewardMoney : ''"
              :lastData="indexData.last"
            />
          </a-col>
          <a-col :sm="24" :md="12" :xl="6">
            <target-data
              title="全约/网签主播流水（元）"
              :newData="indexData.current.signedRewardMoney"
              :oldData="indexData.last ? indexData.last.signedRewardMoney : ''"
              :lastData="indexData.last"
            />
          </a-col>
        </a-row>
...
import {
  ...
  liveAnalyzeInfluencerChart,
  getLiveAnalyzeInfluencerList,
  getLiveAnalyzeOperateList,
  liveAnalyzeOperateChart,
  getDepartmentChild,
  getLiveAnalyzeDepartmentList,
  liveAnalyzeDepartmentChart,
  getLiveAnalyzefieldList,
  getRewardList,
  getIndexLine,
  getFieldList,
  getLiveAnalyzeTarget } from '@/api/report'
export default {
  name: 'DataComp',
  components: {
    TabContent,
  ...
  methods: {
    getTarget () { // 指标统计
      getLiveAnalyzeTarget(this.data).then(res => {
        this.indexData = res
      })
    },
    exportData () { // 导出数据
      ...
      window.location.href = `${process.env.VUE_APP_API_BASE_URL}/report/reward/export?${exportUrl}`
    },
    ...
    getGroupfileList () { // 获取 按组织、按运营、按主播折线图字段   按组织、按运营 type: 1;按主播 type: 2
      getLiveAnalyzefieldList({ type: 1 }).then(res => {
        this.groupTab = res
    ...
    getGroupLine (searchData) { // 按组织折线图
      ...
      liveAnalyzeDepartmentChart(params).then(res => {
        if (this.groupIds.length <= 0 && this.groupsFirstLoad) {
          this.groupsFirstLoad = false
    ...
    getGroupList (params) { // 按组织数据表
      ...
      return getLiveAnalyzeDepartmentList(parameter).then(res => {
        // this.groupIds = res.ids
        if (this.groupIds.length <= 0 && this.groupsFirstLoad) {
          this.groupsFirstLoad = false
          this.groupIds = res.ids
    ...
    getOperatorLine (searchData) { // 按运营折线图
      ...
      liveAnalyzeOperateChart(params).then(res => {
    ...
    getOperatorList (params) { // 按运营数据表
      ...
      return getLiveAnalyzeOperateList(parameter).then(res => {
        if (this.operatorIds.length <= 0 && this.operatorFirstLoad) {
          this.operatorFirstLoad = false
          this.operatorIds = res.ids.map(item => ({
            label: item.name,
    ...
    getInfluencerfileList () { // 获取 按组织、按运营、按主播折线图字段   按组织、按运营 type: 1;按主播 type: 2
      getLiveAnalyzefieldList({ type: 2 }).then(res => {
    ...
    getInfluencerLine (searchData) { // 按主播折线图
      ...
      liveAnalyzeInfluencerChart(params).then(res => {
    ...
    getInfluencerList (params) { // 按主播数据表
      ...
      return getLiveAnalyzeInfluencerList(parameter).then(res => {
        if (this.influencerIds.length <= 0 && this.influencerFirstLoad) {
          this.influencerFirstLoad = false
          this.influencerIds = res.ids.map(item => ({
            label: item.name,
  ...
  watch: {
    data () {
      this.groupIds = this.operatorIds = this.influencerIds = []
      this.groupsFirstLoad = this.influencerFirstLoad = this.operatorFirstLoad = true
      this.getTarget()
      this.getStructureTreeHandle()
      if (this.$refs.table1) {
