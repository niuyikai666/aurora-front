    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline" :form="form" @submit="searchHandle">
        <a-row :gutter="60">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item label="拉新任务类型" class="label-max-left">
              <a-select placeholder="请选择" v-decorator="['missionType']">
                <a-select-option :value="li.value" v-for="li in taskType" :key="li.value">{{ li.name }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
...
<script>
import { importTaskType } from '@/api/commission-video'
import importModal from './importModal'
import exemptModal from './exemptModal'
import moment from 'moment'
import { mapGetters } from 'vuex'
import { STable } from '@/components'
import { TaskTypeColumns } from '../tableColumns'
export default {
  props: {
    fn: {
      type: Function,
  ...
  data () {
    return {
      ...
      errorUrl: process.env.VUE_APP_API_BASE_URL + '/wm/actor/exportErro',
      templateUrl: process.env.VUE_APP_API_BASE_URL + '/wm/actor/template',
      taskType: [{
        name: '拉新',
        value: 1
      }, {
