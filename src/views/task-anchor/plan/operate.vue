<template>
  <div>
    ...
    <div style="background-color:#fff; padding: 30px 24px 6px 24px; margin-top:24px;">
          ...
          <a-row :gutter="60">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="讲师">
                <a-input placeholder="请输入" v-decorator="['lecturerName']" />
              </a-form-item>
            </a-col>
            <a-col :md="8" :sm="24">
    ...
    >
      ...
      <div class="flex-box">
        <div class="box-item" style="color:#303133">
          <select-comp v-if="activeKey !== 'exchange'" :type="activeKey" @change="handleStatusChange" />
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      checkArr: [],
      defaultColunm: {
        'base': ['nickName', 'joinGuildDate', 'isSigned', 'effectiveDay', 'liveBroadcastDuration', 'videoContributeDay', 'videoSuperiorCount', 'action'],
        'base-video': ['nickName', 'joinGuildDate', 'isSigned', 'effectiveDay', 'liveBroadcastDuration', 'videoContributeDay', 'videoSuperiorCount', 'recommendTenHundredsCount', 'recommendTenThousandsCount', 'recommendHundredThousand', 'action'],
        'outer': ['nickName', 'joinGuildDate', 'isSigned', 'supportBeginTime', 'supportEndTime', 'supportMonth', 'effectiveDay', 'action'],
        'video': [ 'nickName', 'joinGuildDate', 'isSigned', 'manyPeopleVideoFlow', 'guildReward', 'otherGuildReward', 'manyPeopleVideoAverageFlow', 'manyPeopleVideoAverageFlowLastMonth', 'manyPeopleVideoAverageFlowIncrease', 'effectiveDay', 'liveBroadcastDuration' ],
        'vertical': ['nickName', 'isMature', 'validDays', 'actorLiveFlow', 'operatorCommission', 'operatorEmployeeName', 'signedEmployeeName', 'recruitEmployeeName', 'lecturerEmployeeName', 'action'],
        'exchange': ['nickName', 'exchangeDate', 'thirtyDaysEndTime', 'startDate', 'adminRewardOne', 'adminRewardTwo', 'signedEmployeeName', 'recruitEmployeeName']
      }
    }
  },
  mounted () {
    this.columns = [ ...this.tabList[0].columns, ...this.extraColumns ]
    this.columns = this.columns.filter(item => item.dataIndex !== 'groupLeaderName')
  },
  methods: {
    getData (parameter) {
    ...
    onTabChange (key) {
      ...
      // 如果是运营页面 视频宫格列表 则过滤掉管理员页面所不需要的字段
      if (this.activeKey === 'video') {
        columns = this.tabInfo.columns.filter(item => item.dataIndex !== 'companyName' && item.dataIndex !== 'companyLeaderName' && item.dataIndex !== 'videoPalaceCommission' && item.dataIndex !== 'presidentCommissionRatio')
      } else {
        columns = this.tabInfo.columns
      }
      this.columns = [ ...columns, ...this.extraColumns ]
      this.columns = this.columns.filter(item => item.dataIndex !== 'groupLeaderName' && item.dataIndex !== 'companyName' && item.dataIndex !== 'companyLeaderName' && item.dataIndex !== 'videoPalaceCommission' && item.dataIndex !== 'presidentCommissionRatio')
    ...
    checkedChange () {
      this.tableColums = this.columns.filter(item => this.colunmsChecked.includes(item.dataIndex) || this.defaultColunm[this.activeKey].includes(item.dataIndex))
      localStorage.setItem(`anchor-${this.activeKey}`, JSON.stringify(this.colunmsChecked))
    },
    handleExport () {
      if (!this.exportUrl) {
  ...
  watch: {
    ...
    columns (newValue) {
      ...
      this.checkArr = newValue.filter(item => !this.defaultColunm[this.activeKey].includes(item.dataIndex))
      this.tableColums = newValue.filter(item => colunms.includes(item.dataIndex) || this.defaultColunm[this.activeKey].includes(item.dataIndex))
    }
  }
}
</script>
