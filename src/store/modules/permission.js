import { asyncRouterMapVertical, constantRouterMap } from '@/config/router.config'
    ...
    if (ele.children) {
      if (ele.meta && ele.meta.title) {
        ele.children.forEach(it => {
          it.meta.parentTitle = ele.meta.title
        })
      }
  ...
  actions: {
    GenerateRoutes ({ commit }) {
      return new Promise(resolve => {
        ...
        getMenusMine().then(res => {
          const { routesVertical } = createMenuByDirection(res)
          // permission纵向菜单
          const permissionRoutesVertical = makePermissionRouters(routesVertical, asyncRouterMapVertical)
          const accessedRoutesVertical = filterAsyncRoutes(permissionRoutesVertical, ['roles'])
          // console.log(accessedRoutesVertical)
          accessedRoutesVertical[0].children = setRouteItemRedirect(accessedRoutesVertical[0].children)
          accessedRoutesVertical[0].redirect = accessedRoutesVertical[0].children[0].path
          // console.log(accessedRoutesVertical)
          commit('SET_ROUTERS', {
            vertical: accessedRoutesVertical,
            horizontal: []
          })
          resolve()
        })
